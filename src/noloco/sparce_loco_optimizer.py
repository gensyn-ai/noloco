...
class PipelineDiLoCoOptimizer:
    ...
    def pick_top_k(self, p: torch.Tensor) -> torch.Tensor:
        k = min(self.top_k, p.numel())
        _, indices = p.view(-1).abs().topk(k, sorted=False)
        mask = torch.zeros(p.numel(), dtype=torch.bool, device=p.device)
        mask[indices] = True
        return mask.view(p.shape)
    def step(self):
        self._step += 1
        ...
        if self._step % self.number_of_local_steps == 0:
            ...
            with torch.no_grad():
                ...
                for grad_gp, gp, p, gm, eb in zip(
                    ...
                    grad_gp.add_(w * (gp - p))
                    grad_gp.add_(eb)
                    top_k_mask = self.pick_top_k(grad_gp)
                    eb.mul_(0.0)
                    eb.add_(grad_gp)
                    eb[top_k_mask] = 0.0
                    grad_gp[~top_k_mask] = 0.0
